...
def extract_fh(year="91", type="FH"):
    mappings = {
...
def extract_uni(year="91", type="Uni"):
    mappings = {
        "Vollstudium": {"keyword": "1", "threshold": 0},
        "Vollstudium_WS_vorgeschrieben": {"keyword": "11", "threshold": 0},
        "Vollstudium_WS_empfohlen": {"keyword": "12", "threshold": 0},
...
def extract_misc(type="Sonstiges", year="91"):
    mappings = {
    ...
    p = 91
    page2 = tx.extract_semi_page(
        base_path, f"Name_{type}_S.{p}", f"Studiengänge_{type}_S.{p-1}", f"Sonstiges_4"
    )
    df1 = pd.concat([page1, page2], axis=1)
    tx = Table_Extractor(
        year,
        type,
        mappings,
    ...
    p = 93
    page2 = tx.extract_semi_page(
        base_path, f"Name_{type}_S.{p}", f"Studiengänge_{type}_S.{p-1}", f"Sonstiges_6"
    )
    df2 = pd.concat([page1, page2], axis=1)
    p = 94
    page1 = tx.extract_semi_page(
        base_path, f"Name_{type}_S.{p}", f"Studiengänge_{type}_S.{p}", f"Sonstiges_7"
    )
    p = 95
    page2 = tx.extract_semi_page(
        base_path, f"Name_{type}_S.{p}", f"Studiengänge_{type}_S.{p-1}", f"Sonstiges_8"
    )
    df3 = pd.concat([page1, page2], axis=1)
    final_df = pd.concat([df1, df2, df3], axis=0)
    tx.df_to_excel_formatting(final_df, custom_name="neue_Länder")
