...
def extract_fh(type="FH", year="84"):
    mappings = {
...
def extract_uni(type="Uni", year="84"):
    mappings = {
...
def extract_misc(type="Sonstiges", year="84"):
    mappings = {
    ...
    df = tx.extract_semi_page(
        base_path, f"Name_{type}_S.{p}", f"Studiengänge_{type}_S.{p}", f"Sonstiges"
    )
