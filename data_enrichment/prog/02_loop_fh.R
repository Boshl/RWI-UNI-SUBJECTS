...
#
# Filename of Code: 02_loop_fh.R
#
# Filename(s) of Input-File(s): - ags.xlsx (AGS/municipality-name key)
#                               - FH_71-96.xlsx (raw StudiBUCH data on study
#                                 subjects at technical colleges from 1971 to 
#                                 1996)
#
# Filename(s) of Output-File(s): "fh_results.xlsx"
#
# Short description: Reads in all raw data from StudiBUCH for technical colleges
#                    and summarizes them.
#
#
...
# Read data for FH for all years (1971-1996) in loop 
for (jahr in jahre) {
  # Create filenames
  dateiname <- paste0("FH_", jahr, ".xlsx")
  
  # Create path to file
  pfad <- paste0("input_tables\\FH\\", jahr, "\\", dateiname)
  ...
  # Add new first column
  df_neu <- cbind(Typ = rep("FH", nrow(df_neu)), df_neu)
  df_neu$neue_spalte <- paste0(df_neu$Stadt, " FH")
...
# Export the results
writexl::write_xlsx(ergebnisse,"data_output\\fh_results.xlsx" )
